...
public class commandRemoveCoin implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String arg, String[] label) {
		...
		if (label.length == 2) {
			...
			Coin.removeCoin(target, money);
			player.sendMessage("§bTu a bien remove §4" + money + " §bValaCoin a " + target.getName());
			...
			player.sendMessage("§bessaye avec la command §6/removecoin \"Nom du player\" \"Nombre de coin a remove\"");
		}

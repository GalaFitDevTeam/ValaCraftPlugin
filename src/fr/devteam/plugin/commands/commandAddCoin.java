...
public class commandAddCoin implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String arg, String[] label) {
		...
		if (label.length == 2) {
			...
			Coin.addCoin(target, money);
			player.sendMessage("§bTu a bien ajouter §4" + money + " §bValaCoin a " + target.getName());
			...
			player.sendMessage("§bessaye avec la command §6/addcoin \"Nom du player\" \"Nombre de coin a add\"");
		}
